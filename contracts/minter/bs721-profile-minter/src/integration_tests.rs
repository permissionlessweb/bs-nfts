...
use bs721_profile::{ExecuteMsg as Bs721ProfileExecuteMsg, SudoParams as Bs721SudoParams};
use bs_profile::{
    common::SECONDS_PER_YEAR,
    market::{
        state::Bid, state::SudoParams as ProfileMarketplaceParams,
    ...
    minter::{
        BsProfileMinterQueryMsg, SudoParams as NameMinterParams, PUBLIC_MINT_START_TIME_IN_SECONDS,
    },
    BsProfileExecuteMsg, BsProfileQueryMsg,
};
...
use profile_marketplace::helpers::get_char_price;
...
pub fn contract_marketplace() -> Box<dyn Contract<Empty, Empty>> {
    ...
    .with_sudo(profile_marketplace::contract::sudo);
    Box::new(contract)
}

pub fn contract_collection() -> Box<dyn Contract<Empty, Empty>> {
    let contract = ContractWrapper::new(
        bs721_profile::contract::execute,
        bs721_profile::contract::instantiate,
        bs721_profile::contract::query,
    )
    .with_sudo(bs721_profile::sudo::sudo);
    Box::new(contract)
}

// pub fn contract_whitelist() -> Box<dyn Contract<Empty,Empty>> {
//     let contract = ContractWrapper::new(
...
fn instantiate_contracts(
    ...
    // 1. Instantiate Name Marketplace
    let msg = bs_profile::market::InstantiateMsg {
    ...
    let bs721_collection: Addr = app
        ...
        .query_wasm_smart(
            minter.clone(),
            &bs_profile::minter::BsProfileMinterQueryMsg::Collection {},
        )
        .unwrap();

    let msg = bs_profile::market::ExecuteMsg::Setup {
...
fn mint_and_list(
    ...
    let approve_all_msg: Bs721ProfileExecuteMsg<Empty> = Bs721ProfileExecuteMsg::ApproveAll {
...
mod execute {
    use bs721_profile::QueryMsg as Bs721ProfileQueryMsg;
    use bs_profile::market::state::{Ask, SudoParams};
    ...
    fn test_two_sales_cycles() {
        ...
        let msg: Bs721ProfileExecuteMsg<Empty> = Bs721ProfileExecuteMsg::Approve {
    ...
    fn test_reverse_map() {
        ...
        let res: Result<String, cosmwasm_std::StdError> = app.wrap().query_wasm_smart(
            COLLECTION,
            &(BsProfileQueryMsg::AssociatedAddress {
                name: NAME.to_string(),
            }),
        );
        assert!(res.is_err());

        let msg = BsProfileExecuteMsg::AssociateAddress {
            name: NAME.to_string(),
...
mod query {
    ...
    use bs_profile::market::AskRenewPriceResponse;
    use bs_profile::market::{state::Ask, BidOffset};
    use bs_std::NATIVE_DENOM;
    ...
    //     let msg: Bs721ProfileExecuteMsg<Empty> = Bs721ProfileExecuteMsg::TransferNft {
    ...
    //         .wrap()
    //         .query_wasm_smart(MINTER, &BsProfileMinterQueryMsg::Params {})
    //         .unwrap();
    ...
    //     let params: ProfileMarketplaceParams = app
    //         .wrap()
    //         .query_wasm_smart(MKT, &ProfileMarketplaceQueryMsg::Params {})
    //         .unwrap();
    ...
    //         Addr::unchecked(MKT),
    //         &MarketplaceExecuteMsg::ProcessRenewals {
    ...
    fn query_name() {
        ...
        let user = "bitsong1hsk6jryyqjfhp5dhc55tc9jtckygx0epmnl9d9";
        let cosmos_address = "bitsong1hsk6jryyqjfhp5dhc55tc9jtckygx0epmnl9d9";
...
mod collection {
    use bs721_profile::contract::QueryMsg as Bs721ProfileQueryMsg;
    use bs_profile::{
        market::state::{Ask, SudoParams},
    ...
    pub(crate) fn transfer(app: &mut TestApp, from: &str, to: &str) {
        let msg = Bs721ProfileExecuteMsg::<Empty>::TransferNft {
    ...
    fn send(app: &mut TestApp, from: &str, to: &str) {
        ...
        let send_msg = Bs721ProfileExecuteMsg::<Empty>::SendNft {
            contract: target,
            token_id: NAME.to_string(),
            msg,
        };
    ...
    fn verify_twitter() {
        ...
        let msg = BsProfileExecuteMsg::AddTextRecord {
            name: NAME.to_string(),
        ...
        let msg = BsProfileExecuteMsg::VerifyTextRecord {
            name: NAME.to_string(),
    ...
    fn verified_text_record() {
        ...
        let msg = BsProfileExecuteMsg::UpdateTextRecord {
            name: NAME.to_string(),
            record: TextRecord {
                name: name.to_string(),
    ...
    fn burn_nft() {
        ...
        let msg: Bs721ProfileExecuteMsg<Empty> = Bs721ProfileExecuteMsg::Burn {
            token_id: NAME.to_string(),
        };
        let res = app.execute_contract(
            Addr::unchecked(USER),
    ...
    fn sudo_update() {
        ...
        let msg = bs721_profile::SudoMsg::UpdateParams {
...
mod associate_address {
    ...
    fn associate_with_a_contract_with_no_admin_fail() {
        ...
        let res = app
            ...
            .map_err(|e| e.downcast::<bs721_profile::ContractError>().unwrap())
            .unwrap_err();
        assert!(matches!(
            res,
            bs721_profile::ContractError::UnauthorizedCreatorOrAdmin {}
        ))
    }
