...
pub fn execute_mint(
    ...
    Bs721AccountsContract::default().increment_tokens(deps.storage)?;
...
fn reset_token_metadata_and_reverse_map(deps: &mut DepsMut, token_id: &str) -> StdResult<()> {
    let mut token = Bs721AccountsContract::default()
        .tokens
...
fn _transfer_nft(
    ...
    Bs721AccountsContract::default().execute(deps, env, info.clone(), msg)?;
...
pub fn execute_set_profile_marketplace(
    ...
    let minter = Bs721AccountsContract::default().minter(deps.as_ref())?;
...
fn only_owner(deps: Deps, sender: &Addr, token_id: &str) -> Result<Addr, ContractError> {
    let owner = Bs721AccountsContract::default()
        .tokens
        .load(deps.storage, token_id)?
        .owner;
